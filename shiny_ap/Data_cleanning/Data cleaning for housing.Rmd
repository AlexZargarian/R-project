---
title: "Data Cleaning for Housing Prices"
...
This script cleans and transforms Airbnb housing data. The process involves selecting relevant columns, removing unnecessary ones, cleaning data types, handling missing values, creating new features, and renaming columns for clarity before exporting the final dataset.

## Setup and Library Loading

This chunk sets up the environment and loads the necessary R libraries for data manipulation and analysis.
...
Here, we load the original dataset from the specified CSV file and display its initial dimensions and column names.

```{r load-data}
# Define the file path for the raw data.
# Please ensure this path is correct for your system.
file_path <- "/Data used/House Price/part-00295-e039244b-cf18-4c20-a539-6e32f86875aa.c000.csv"
...
This section performs the main data cleaning tasks. We select a subset of columns, clean text and numeric data, handle boolean conversions, and create a new calculated column for price per guest.

```{r clean-and-transform-data}
# Clean and process the data using a dplyr pipeline.
cleaned_data <- original_data %>%
  # Step 2.1: Select only the columns we need for the final analysis.
  # Columns like 'amenities', 'min_nights', 'rating_cleanliness', and various revpar/revenue metrics are removed as requested.
  select(
    listing_id,
    listing_name,
    listing_type,
  ...
  # Step 2.2: Mutate columns to clean data, convert types, and create new features.
  mutate(
    ...
    # Convert currency and percentage columns to numeric, removing non-numeric characters.
    ...
    # Convert occupancy rates from string (e.g., "50%") to numeric (e.g., 0.50).
    ttm_occupancy = as.numeric(gsub("%", "", ttm_occupancy)) / 100,
    l90d_occupancy = as.numeric(gsub("%", "", l90d_occupancy)) / 100,

    # Ensure other numeric columns are of the correct type.
    ...
    # Validate rating scores to be within the 0-5 range.
    ...
    # Create a new feature for price per guest. Use pmax to avoid division by zero.
    price_per_guest = ttm_avg_rate / pmax(guests, 1, na.rm = TRUE),
    
    # Create a location category based on coordinates for Yerevan.
  ...
  # Step 2.3: Filter out rows with essential missing information.
  filter(
    !is.na(listing_id),
...
To make the dataset easier to understand, we rename the columns to be more descriptive.
...
Here we'll print the first few rows of the final, cleaned dataframe to see the result.
...
output_path <- "/shiny_ap/data/cleaned_rental_data_from_Airbnb_v2.csv"
